...
class MemberUserProfile : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_member_user_profile)
    ...
    private fun loadProfile(userID:String){
        ...
        var userRoleET = findViewById<EditText>(R.id.superAdminUserProfile_userRole2)
                ...
                if(snapshot.exists()){
                    for(i in snapshot.children){
                        if(i.child("user_uid").value.toString() == userID) {
                            if (!this@MemberUserProfile.isFinishing){
